...
class WPLib {
	...
	static function the_partial_html( $partial_slug, $_partial_vars = array(), $item = null ) {
		...
		if ( ! $partial->found ) {
			...
			self::$_file_loading = $_save_filename = $partial->filename;

			$partial->called_by = $method = ( 
				( $item && $var_name = $item->get_constant( 'VAR_NAME' ) ) 
	                ? "${$var_name}->" 
	                : get_called_class() . '::' 
                ) . __FUNCTION__ . '()';
			...
			if ( ! is_object( $partial ) || $method !== $partial->called_by ) {
				/*
				 * Had to use $_save_filename because self::$_file_loading gets cleared if
				 * $partial is assigned a string inside $partial->filename. Why? Dunno.
				 */
				$err_msg = sprintf( __( 'Variable $partial overritten in %s. Error reported ', 'wplib' ), $_save_filename );

				WPLib::trigger_error( $err_msg, E_USER_ERROR );
	 ...
	 * @return string
	 */
	static function args_to_err_msg( $args = array() ) {
	...
	static function get_list( $query = array(), $args = array() ) {
		...
		$query = is_array( $query )
			? self::args_to_err_msg( $query )
			: null;

		$args = is_array( $args )
			? self::args_to_err_msg( $args )
			: null;
